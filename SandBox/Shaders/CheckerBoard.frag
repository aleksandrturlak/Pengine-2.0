...
void main()
{
	MaterialBufferReference materialBufferReference = MaterialBufferReference(materialBuffer);
	DefaultMaterial material = materialBufferReference.material;

	ivec2 coords = ivec2(materialBufferReference.size * uv) % 2;

	vec4 checkerBoardColor = materialBufferReference.secondColor;
	if (coords.x == coords.y)
	{
		checkerBoardColor = materialBufferReference.firstColor;
	}
	...
	vec4 albedoColor = checkerBoardColor * texture(bindlessTextures[material.albedoTexture], finalUV) * material.albedoColor * color;
